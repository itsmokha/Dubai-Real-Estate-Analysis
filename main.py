...
# print (results_soup.select("[type='application/ld+json']"))
...
for pageNumber in tqdm(range(1,pages)): 
    try:
        ...
        results = requests.get(url,headers=headers)
        results_soup = soup(results.text, 'lxml')
        info = results_soup.select("[type='application/ld+json']")[0]
...
print(df)
df.to_csv('data-geo-2.csv', index=False)

# for i in range (1,24):
#     l.append(scrape(results_soup.select("[type='application/ld+json']")[i]))

# print(l)

# info = results_soup.select("[type='application/ld+json']")[0]
# numberOfEntries = json.loads(info.text)['numberOfItems']
# info = results_soup.select("[type='application/ld+json']")[20]
